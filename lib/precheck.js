...
export async function resolveTarget(spec, options = {}) {
  ...
  try {
    ...
    return {
      ...
      // Size and release date ride along with the packument we already have, so
      // the report can say how big and how old this version is at no extra cost.
      unpackedSize: manifest.dist?.unpackedSize,
      fileCount: manifest.dist?.fileCount,
      publishedAt: packument.time?.[pick] ?? packument.time?.modified,
    }
  } catch (error) {
...
export function analyze(input) {
  ...
  // How big it is and how old it is. Neither changes the verdict (they stay
  // informational), but both come free with the packument we already fetched.
  const size = describeSize(target.unpackedSize, target.fileCount)
  if (size !== undefined) {
    findings.push(
      finding('package-size', 'info', '解包体积', `装完后大约占这么多磁盘：${size}。`, {
        required: '解包体积',
        actual: size,
      }),
    )
  }
  const age = describeAge(target.publishedAt, version)
  if (age !== undefined) {
    findings.push(
      finding('package-stale', 'info', '最近发布', `这个版本的发布时间：${age}。`, {
        required: '发布时间',
        actual: age,
      }),
    )
...
/**
 * Human-readable byte size, or undefined when the registry did not report one.
 * @param bytes - `dist.unpackedSize` from the packument.
 * @param files - `dist.fileCount`, when present.
 * @returns e.g. `1.2 MB（340 个文件）`.
 */
export function describeSize(bytes, files) {
  ...
  const suffix = typeof files === 'number' && files > 0 ? `（${String(files)} 个文件）` : ''
  return `${rounded} ${units[unit]}${suffix}`
}

/**
 * How long ago a release was published, or undefined without a usable date.
 * @param publishedAt - an ISO date from the packument's `time` map.
 * @param version - the version it belongs to, shown alongside.
 * @returns e.g. `42 天前（v0.8.4）`.
 */
export function describeAge(publishedAt, version) {
  if (typeof publishedAt !== 'string' || publishedAt === '') return undefined
  const then = Date.parse(publishedAt)
  if (Number.isNaN(then)) return undefined
  const days = Math.floor((Date.now() - then) / 86400000)
  const label = version === undefined ? '' : `（v${String(version)}）`
  if (days <= 0) return `今天${label}`
  if (days === 1) return `昨天${label}`
  // Days up to a quarter stay in days: "42 天前" is more useful than "1 个月前".
  if (days < 90) return `${String(days)} 天前${label}`
  if (days < 365) return `${String(Math.floor(days / 30))} 个月前${label}`
  return `${(days / 365).toFixed(1)} 年前${label}`
}
