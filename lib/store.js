...
export async function listBackups(env = process.env) {
  ...
  for (const entry of entries) {
    ...
    const manifest = await readDocument(join(root, entry.name, 'manifest.json'), undefined)
    if (manifest !== undefined) out.push({ id: entry.name, ...manifest, kind: manifest.kind ?? 'manual' })
  }
